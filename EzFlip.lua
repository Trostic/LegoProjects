-- ui library by dawid, script by trostic. enjoy
...
spawn(function()
    while true do
        wait()
        if getgenv().automoneyclick == true then
...
main:Button("Equip All Hats",function()
    for i, v in pairs(game:GetService("Players").LocalPlayer.PlayerGui.Shop.Main.Frames.Inventory.ScrollingFrame:GetChildren()) do
        if not game.Players.LocalPlayer.Character:FindFirstChild(v.Name) then
        local args = {
            [1] = v.Name
        }
        
        game:GetService("ReplicatedStorage").Events.equip:InvokeServer(unpack(args))
        
        end
    end
end)
main:Button("Unequip All Hats",function()
...
getgenv().rigged = false
main:Button("Rig Roulette / Always win",function(v)
    if getgenv().rigged == false then
    getgenv().rigged = true
    game:GetService("ReplicatedStorage").Events.roulette_info.OnClientEvent:Connect(function(s,c)
        if s == "Spin" then 
            game:GetService("ReplicatedStorage").Events.roulette_request:InvokeServer("Betting",game.Players.LocalPlayer.Cash.Value,c)
        else
            return
        end
...
main:Toggle("Auto Sell INVENTORY",false,function(v)
